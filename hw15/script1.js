...
document.querySelector(".goButton").addEventListener("click", function () {
  ...
  const checkUser = validUserData(
    userName,
    document.forms.someForm.elements[0]
  );

  const userFatherName = document.forms.someForm.elements.fatherName.value;
  const checkFatherName = validUserData(
    userFatherName,
    document.forms.someForm.elements[1]
  );
  const userSurname = document.forms.someForm.elements.surname.value;
  const checkSurname = validUserData(
    userSurname,
    document.forms.someForm.elements[2]
  );

  const userCity = document.querySelector("select[name=city]").value;
  const checkUserCity = validUserCityAndWarehouse(
    userCity,
    document.forms.someForm.elements[3]
  );
  const userWarehouse = document.querySelector("select[name=warehouse]").value;
  const checkUserWarehouse = validUserCityAndWarehouse(
    userWarehouse,
    document.forms.someForm.elements[4]
  );

  const payMethod = document.querySelectorAll("input[type=radio]");
  let finalPayMethod = validPayMethod(payMethod);
  const payMethodForUserInfo = findValuePayMethod(finalPayMethod);
  ...
  const checkQuantity = validQuantity(
  ...
  if (
    ...
    orderInfo.children[4].textContent = `Warehouse of Nova Pochta: ${userWarehouse}`;
    ...
    orderInfo.children[7].textContent = `Your comment is: ${userComment}`;
  }
});
